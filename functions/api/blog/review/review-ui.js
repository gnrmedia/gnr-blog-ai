...
function buildJs() {
  ...
  return [
    ...
    "  var token = (window.__REVIEW_TOKEN__ || '').toString().trim();",
    "  if(!token){ console.error('Missing __REVIEW_TOKEN__'); return; }",
    "",
    ...
    "      }",
    "",
    "      var row = dbg.json && dbg.json.row ? dbg.json.row : null;",
    "      var status = row && row.status ? String(row.status).toUpperCase() : 'UNKNOWN';",
    "      var expiresAt = row && row.expires_at ? String(row.expires_at) : '';",
    "      var draftId = row && row.draft_id ? String(row.draft_id) : '';",
    "",
    ...
    "        if(previewEl){",
    "          // Fetch HTML preview and extract body content (safe best-effort).",
    "          try{",
    "            var pr = await fetch(location.origin + '/api/blog/draft/render/' + encodeURIComponent(draftId) + '?view=generic', { method:'GET', headers:{ 'Cache-Control':'no-cache' } });",
    "            var html = await pr.text();",
    "            if(pr.ok && html){",
    "              // Extract the inside of <body> if present; otherwise drop in raw",
    "              var m = html.match(/<body[^>]*>([\\s\\S]*?)<\\/body>/i);",
    "              previewEl.innerHTML = m ? m[1] : html;",
    "            } else {",
