...
"            }",
    ...
    "          try{",
"          // If image came from file upload (data:image), upload to Cloudflare Images",
"          if(/^data:image\\//i.test(u) && clientImageFile && clientImageFile.files && clientImageFile.files[0]){",
"            var fd = new FormData();",
"            fd.append('t', token);",
"            fd.append('visual_key', vk);",
"            fd.append('file', clientImageFile.files[0]);",
"",
...
"            }",
"",
"            // Replace URL field with canonical https URL",
"            if(parsed.json && parsed.json.image_url){",
"              clientImageUrl.value = parsed.json.image_url;",
"            }",
"          } else {",
"            // HTTPS URL swap (no upload)",
...
"      }",
    ...
    "          if(!/^https:\\/\\//i.test(u) && !/^data:image\\//i.test(u)){ showToast('Image URL must start with https:// (or upload a file).', 'error'); return; }",
