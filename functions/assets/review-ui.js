...
"  }",
"",
"  function stripEditorVisualMarkers(md){",
"    // Editor-only: hide internal VISUAL placeholders from clients (do NOT show in textarea).",
"    // This intentionally removes ONLY VISUAL markers so normal HTML comments remain intact.",
"    try {",
"      var s = String(md || '');",
"      // Remove whole-line markers like: <!-- VISUAL:cta_banner --> (any VISUAL:key)",
"      // Handles optional surrounding whitespace and optional trailing newline.",
"      s = s.replace(/^\\s*<!--\\s*VISUAL:[^>]*-->\\s*\\n?/gmi, '');",
"      // Also catch inline occurrences (rare) without nuking surrounding text:",
"      s = s.replace(/\\s*<!--\\s*VISUAL:[^>]*-->\\s*/gmi, ' ');",
"      return s;",
"    } catch (_) {",
"      return String(md || '');",
"    }",
...
"        draftText.value = stripEditorVisualMarkers(stripLeadingAiMeta(md));",
"        try { draftText.__gnrSanitized = true; } catch (_) {}",
"        draftPrefilled = true;",
"      }",
    ...
    "          editing = !editing;",
    "          setEditing(editing);",
    "            // One-time editor sanitize (hide internal VISUAL markers)",
"            try{",
"              if(!draftText.__gnrSanitized){",
"                draftText.value = stripEditorVisualMarkers(String(draftText.value || ''));",
"                draftText.__gnrSanitized = true;",
"              }",
"            }catch(_){ }",
