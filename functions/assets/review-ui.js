...
function buildJs() {
  ...
  return [
    ...
    "  // Detect whether server already prefilled draft markdown into the textarea",
"  var draftPrefilled = false;",
"  try { draftPrefilled = !!(draftText && String(draftText.value || '').trim()); } catch (_) { draftPrefilled = false; }",
"",
    ...
    "",
    "  // ------------------------------------------------------------",
"  // Modal readability fix (LOCKED): ensure Confirm Acceptance text is visible",
"  // This is CSS-only and does not change modal behavior.",
"  // ------------------------------------------------------------",
"  try {",
"    if (modalBack && !document.getElementById('gnrModalReadabilityFix')) {",
"      var st = document.createElement('style');",
"      st.id = 'gnrModalReadabilityFix';",
"      st.textContent = [",
"        '#modalBack{display:none;align-items:center;justify-content:center;}',",
"        '#modalBack{background:rgba(0,0,0,.62)!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;}',",
"        '#modalBack *{color:#fff!important;}',",
"        // Ensure the inner modal card has a solid background even if it inherits transparency",
"        '#modalBack > *{background:rgba(17,17,17,.94)!important;}',",
"        // If your modal has nested wrappers, force the common containers too",
"        '#modalBack .modal,#modalBack .card,#modalBack .dialog,#modalBack .modalContent{background:rgba(17,17,17,.94)!important;}',",
"        // Make the heading readable (the text you said you can’t see)",
"        '#modalBack h1,#modalBack h2,#modalBack h3{color:#fff!important;opacity:1!important;}',",
"        '#modalBack p,#modalBack span{color:#fff!important;opacity:1!important;}',",
"        // Make buttons readable and prevent background blur washing them out",
"        '#modalBack button{opacity:1!important;}',",
"      ].join('');",
...
"",
    ...
    "    if(elStatusText) elStatusText.textContent = (s || 'UNKNOWN').replace(/_/g, ' ');",
    ...
    "      else if(s==='PUBLISHED'){ color='#22c55e'; glow='rgba(34,197,94,.14)'; }",
    "      else if(s==='APPROVED' || s==='ACCEPTED'){ color='#f59e0b'; glow='rgba(245,158,11,.18)'; }",
...
"",
"      // Make sure the TOP of the article is visible when Edit Mode opens",
"      try { draftText.scrollTop = 0; } catch (_) {}",
"      try { if (typeof draftText.setSelectionRange === 'function') draftText.setSelectionRange(0, 0); } catch (_) {}",
"      try { draftText.focus(); } catch (_) {}",
"",
"      // Also bring the editor section into view (so you’re not stuck in preview land)",
"      try { editorWrap.scrollIntoView({ behavior:'smooth', block:'start' }); } catch (_) {}",
"    } else {",
...
"  }",
    ...
    "  }",
    "",
"  function stripLeadingAiMeta(md){",
"    // Remove leading HTML comment metadata blocks (client should never see these in Edit Mode).",
"    // Only strips contiguous <!-- ... --> blocks at the very start of the markdown.",
"    try {",
"      var s = String(md || '');",
...
"",
    ...
    "  // ------------------------------------------------------------",
"  // Published preview loader (reusable) + reload hook",
"  // ------------------------------------------------------------",
"  var __gnrPreviewDraftId = '';",
"  function __gnrRenderUrl(draftId){",
"    // cache-bust so mobile preview refreshes without leaving the page",
"    return location.origin + '/api/blog/draft/render/' + encodeURIComponent(draftId) +'?view=generic&embed=1' +'&cb=' + Date.now();}",
"  async function __gnrLoadPublishedPreview(){",
"    try{",
"      var draftId = String(__gnrPreviewDraftId || '').trim();",
"      if(!draftId) return;",
"      var previewEl = document.getElementById('publishedPreview');",
"      if(!previewEl) return;",
"      var renderUrl = __gnrRenderUrl(draftId);",
"      // Wire the \"Open full preview\" button (new tab) – no cache-buster needed",
"      var openBtn = document.getElementById('openFullPreviewBtn');",
"      if(openBtn){",
"        openBtn.href = renderUrl.replace('&embed=1','').replace(/&cb=\\d+/, '');",
"      }",
...
"          /* Scrollbar on black frame (Chrome / Edge) */",
"          '.phoneScreen::-webkit-scrollbar{width:12px;}' +",
"          '.phoneScreen::-webkit-scrollbar-track{background:#000;}' +",
"          '.phoneScreen::-webkit-scrollbar-thumb{background:#fff;border-radius:10px;border:3px solid #000;}' +",

"          /* Firefox scrollbar */",
"          '.phoneScreen{scrollbar-color:#fff #000;scrollbar-width:auto;}' +",

"          /* White screen area fills available width */",
...
"          '.phoneScreen .gnr-render-root main,' +",
"          '.phoneScreen .gnr-render-root article,' +",
"          '.phoneScreen .gnr-render-root section,' +",
"          '.phoneScreen .gnr-render-root .container,' +",
"          '.phoneScreen .gnr-render-root .content,' +",
"          '.phoneScreen .gnr-render-root .wrapper,' +",
"          '.phoneScreen .gnr-render-root > *{' +",
"            'max-width:none !important;width:100% !important;margin:0 !important;' +",
"          '}' +",
"          /* ── Renderer-specific overrides (kill gutter from blog-handlers CSS) ── */",
"          '.phoneScreen .gnr-render-root .gnr-wrap{' +",
"            'max-width:none !important;width:100% !important;' +",
"            'margin:0 !important;padding:0 !important;' +",
"          '}' +",
"          '.phoneScreen .gnr-render-root .gnr-article{' +",
"            'border:none !important;border-radius:0 !important;' +",
"            'margin:0 !important;padding:0 !important;' +",
"          '}' +",
"          '.phoneScreen .gnr-render-root .gnr-head{' +",
"            'padding:12px 10px 6px !important;' +",
"          '}' +",
"          '.phoneScreen .gnr-render-root .gnr-body{' +",
"            'padding:6px 10px 12px !important;' +",
"          '}' +",
"          '.phoneScreen .gnr-render-root h1,' +",
"          '.phoneScreen .gnr-render-root h2,' +",
"          '.phoneScreen .gnr-render-root h3{' +",
"            'max-width:100%;overflow-wrap:anywhere;word-break:break-word;hyphens:auto;' +",
"            'line-height:1.12;letter-spacing:-0.02em;margin:18px 0 10px;' +",
"          '}' +",
"          '.phoneScreen .gnr-render-root h1{font-size:28px !important;color:#111 !important;}' +",
"          '.phoneScreen .gnr-render-root h2{font-size:22px !important;color:#111 !important;}' +",
"          '.phoneScreen .gnr-render-root h3{font-size:18px !important;color:#111 !important;}' +",
"          '.phoneScreen .gnr-render-root p,' +",
"          '.phoneScreen .gnr-render-root li{font-size:16px !important;line-height:1.6;}' +",

"          '.phoneScreen .gnr-render-root img{max-width:100%;height:auto;display:block;}' +",
"          '.phoneScreen .gnr-render-root a{color:#0b5fff !important;text-decoration:underline;}' +",
"        '</style>';",

"        previewEl.innerHTML = styleTag + '<div class=\"gnr-render-root\">' + bodyHtml + '</div>' + mobileOverride;",
"      } else {",
...
"    }catch(e){",
"      try{",
"        var previewEl2 = document.getElementById('publishedPreview');",
"        if(previewEl2) previewEl2.innerHTML = '<div style=\"padding:14px;border-radius:16px;border:1px dashed rgba(0,0,0,.18);background:#fff;\">Preview unavailable.</div>';",
"      }catch(_){ }",
"    }",
"  }",
...
"  async function boot(){",
    ...
    "        '';",
"      var md = (dbg.json && dbg.json.draft_markdown) ? String(dbg.json.draft_markdown) : '';",
"      if(draftText && md && md.trim()){",
"        draftText.value = stripLeadingAiMeta(md);",
"        draftPrefilled = true;",
"      }",
    ...
    "      // We reuse the shared loader so Save can refresh it without leaving the page.",
    "      if(draftId){",
    "        __gnrPreviewDraftId = draftId;",
    "        try { await __gnrLoadPublishedPreview(); } catch (_) {}",
    "      }",
...
"            // Only clear if the textarea was NOT server-prefilled with markdown",
"            if(!draftPrefilled){",
"              var v = String(draftText.value || '').trim();",
"              if(!v || v==='Loading draft…' || v==='Loading draft...'){",
"                draftText.value = '';",
...
"            // Keep editor open (don’t bounce the user out)",
"            try { if (!editing) { editing = true; setEditing(true); } } catch (_) {}",
"            // Reload published preview (if a helper exists); otherwise do nothing",
"            try { if (window.__gnrReloadPublishedPreview) window.__gnrReloadPublishedPreview(); } catch (_) {}",
"          } finally {",
...
"      }",
    "",
    "      // Client image upload: fill URL field ONLY. Do NOT auto-preview until Preview is clicked.",
    "      var clientImageFile = document.getElementById('clientImageFile');",
    "      if(clientImageFile){",
    "        clientImageFile.addEventListener('change', function(e){",
    "          try{",
    ...
    "              showToast('Image is too large (>2MB). Please resize before uploading.', 'error');",
    "              return;",
    "            }",
    "            var reader = new FileReader();",
    "            reader.onload = function(){",
    "              var dataUrl = String(reader.result || '').trim();",
    "              if(!/^data:image\\//i.test(dataUrl)){",
    "                showToast('Upload failed: unexpected data format.', 'error');",
    "                return;",
    "              }",
    ...
    "                clientImagePreviewEmpty.style.display = 'block';",
    "                clientImagePreviewEmpty.textContent = '(ready — click Preview)';",
    "              }",
    "              showToast('Image loaded. Click Preview to view, then Save image.', 'success');",
    "            };",
...
"            clientPreviewVisualBtn.textContent = 'Hide preview';",
"            previewVisible = true;",
"          } else {",
"            if(clientImagePreview) clientImagePreview.style.display = 'none';",
"            if(clientImagePreviewEmpty){",
...
"              clientImagePreviewEmpty.textContent = '(ready — click Preview)';",
"            }",
"            clientPreviewVisualBtn.textContent = 'Preview';",
...
"          var clientImageFile = document.getElementById('clientImageFile');",
"          var f = (clientImageFile && clientImageFile.files && clientImageFile.files[0]) ? clientImageFile.files[0] : null;",
"",
"          if(!vk){ showToast('Choose an image slot first.', 'error'); return; }",
"          if(!u){ showToast('Paste an image URL first (or upload a file).', 'error'); return; }",
"          if(!/^https:\\/\\//i.test(u) && !/^data:image\\//i.test(u)){",
"            showToast('Image URL must start with https:// (or upload a file).', 'error');",
"            return;",
"          }",
"",
...
"          try{",
"            // File upload path: data:image/* + file present → upload to Cloudflare Images (PUBLIC)",
"            if(/^data:image\\//i.test(u) && f){",
"              var fd = new FormData();",
"              fd.append('t', token);",
"              fd.append('visual_key', vk);",
"              fd.append('file', f);",
"",
"              var res = await fetch(location.origin + '/api/blog/review/visuals/upload', {",
"                method:'POST',",
"                body: fd",
"              });",
"              var parsed = await readJson(res);",
"              if(!parsed.ok){",
"                showToast((parsed.json && (parsed.json.error||parsed.json.detail)) ? (parsed.json.error||parsed.json.detail) : ('Upload failed (HTTP '+parsed.status+')'), 'error');",
"                saveFailed = true;",
"                return;",
"              }",
"              if(parsed.json && parsed.json.image_url){",
"                clientImageUrl.value = String(parsed.json.image_url);",
"              }",
"            } else {",
"              // HTTPS URL swap path (no upload)",
...
"          } finally {",
"            if(saveFailed) clientSaveVisualBtn.disabled = false;",
"          }",
"        });",
"      }",
    "",
...
"          var payload = {",
"            t: token,",
"            client_topic_suggestions: String(futureTopics && futureTopics.value ? futureTopics.value : '').trim(),",
