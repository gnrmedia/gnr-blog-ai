...
"          '.phoneScreen .gnr-render-root{padding:18px 16px 22px;color:#111 !important;}' +",
...
"            'max-width:100%;overflow-wrap:anywhere;word-break:break-word;hyphens:auto;line-height:1.12;letter-spacing:-0.02em;margin:18px 0 10px;' +",
"          '}' +",
...
"          '.phoneScreen .gnr-render-root img{max-width:100%;height:auto;}' +",
...
""
    ...
    "        // existing preview load logic follows",

    "        var previewEl = document.getElementById('publishedPreview');",
    "        if(previewEl){",
    "      // Fetch HTML preview and import styles so it looks like the real article",
    "      try{",
    "        var renderUrl = location.origin + '/api/blog/draft/render/' + encodeURIComponent(draftId) + '?view=generic&embed=1&t=' + encodeURIComponent(token);",
    "      ",
...
"          var styleTag = cssText ? ('<style>' + cssText + '</style>') : '';",
"          // Mobile-only typography override MUST come AFTER render CSS so it wins",
"          var mobileOverride = '<style>' +",
"            '.phoneScreen .gnr-render-root{padding:18px 16px 22px;color:#111 !important;}' +",
"            '.phoneScreen .gnr-render-root *{color:inherit;}' +",
"            '.phoneScreen .gnr-render-root h1,' +",
